/* 문제: 큰 방에 N개의 풍선이 떠있다. 풍선들은 왼쪽부터 오른쪽까지 일렬로 있다. 
...
for (let i = 0; i < n; i++) {
  ...
  const arrows = currentHeights.get(height)
  if (arrows > 0) {
    currentHeights.set(height, arrows - 1)    
  } else {
    count++
  }
  currentHeights.set(height - 1, (currentHeights.get(height - 1) || 0) + 1)
}
