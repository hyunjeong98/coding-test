  ...
  if (isPpalindrome(string)) {
    ...
    for (let i = 0; i < n / 2; i++) {
      if (string[i] !== string[n - 1 - i]) {
        if (isPpalindrome(string.slice(0, i) + string.slice(i + 1))) pseudo = true
        if (isPpalindrome(string.slice(0, n - 1 - i) + string.slice(n - i))) pseudo = true
        break
      }
    }
    if (pseudo) console.log(1)
    else console.log(2)
  }
}
